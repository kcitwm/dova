...
{   //____________________________________________________________________________
    ...
    public  class SocketListener
    {
        ...
        public event EventHandler<ReceiveArgs> DataReceived;

        protected virtual void OnDataReceived(ReceiveArgs e)
        {
        ...
        private void ProcessAccept(SocketAsyncEventArgs acceptEventArgs)
        {
            ...
            if (acceptEventArgs.SocketError != SocketError.Success)
            {
                ...
                Log.Error("SocketError, accept id " + theAcceptOpToken.TokenId);
        ...
        private void StartReceive(SocketAsyncEventArgs receiveSendEventArgs)
        {
            ...
            Log.Info("StartReceive:设置buffer完成");

            // Post async receive operation on the socket.
            bool willRaiseEvent = receiveSendEventArgs.AcceptSocket.ReceiveAsync(receiveSendEventArgs);
            Log.Info("StartReceive:异步接收完成:willRaiseEvent=" + willRaiseEvent);
            ...
            if (!willRaiseEvent)
            {
                ...
                //see in testing.
                Log.Info("StartReceive:准备调用ProcessReceive处理数据");
           ...
           // Log.Info("IO_Completed0");
            ...
            switch (e.LastOperation)
            {
                case SocketAsyncOperation.Receive: 
                   // Log.Info("IO_Completed1");
                    ProcessReceive(e);
                    break;

                case SocketAsyncOperation.Send: 
                   // Log.Info("IO_Completed2");
                    ProcessSend(e);
                    break; 
                default:
        ...
        { 
            ...
            {   
                ...
                //SAEA object.
                receiveSendToken.Reset();
               // Log.Info("ProcessReceive:10");
                if (null != ra.DataToSend)
                {
                    receiveSendToken.theMediator.PrepareOutgoingData(ra.DataToSend);
                    //Log.Info("ProcessReceive:11");

                    StartSend(receiveSendToken.theMediator.GiveBack());
                    //Log.Info("ProcessReceive:12");
                }
                else 
                    CloseClientSocket(receiveSendEventArgs);
