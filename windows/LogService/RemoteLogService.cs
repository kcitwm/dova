namespace WQFree.Utility
{
    ...
    public class RemoteLogService : ILog
    {
        ...
        static SqlBulkCopyColumnMapping[] mappings = new SqlBulkCopyColumnMapping[12];
                    ...
                    while (true)
                    {
                        ...
                        if (ss.Length == 11 || ss.Length == 10)
                        {
                            if (ss.Length == 11)
                            {
                                ...
                                row[9] = ss[8];
                                row[10] = mv;
                                row[11] = ss[10].Length > 256 ? ss[10].Substring(0, 255) : ss[10];
                            }
                            else
                            {
                                mv = ss[8].ToDouble();
                                ...
                                row[9] = ss[7];
                                row[10] = mv;
                                row[11] = ss[9].Length > 256 ? ss[9].Substring(0, 255) : ss[9];
                            }
                            schema.Rows.Add(row);
                        }
                    }
