...
namespace Dova.Cust.Framework.Common
{
    public class HttpClient
    {
        #region fields
        private bool keepContext;
        private string defaultLanguage = Encoding.Default.HeaderName;
        ...
        private HttpWebRequest CreateRequest()
        {
            ...
            if (verb == HttpVerb.POST)
            {
                ...
                else
                {
                    ...
                    foreach (string key in postingData.Keys)
                    {
                        sb.AppendFormat("{0}={1}&", HttpUtility.UrlEncode(key, defaultEncoding), HttpUtility.UrlEncode(postingData[key], defaultEncoding));
                    }
                    if (sb.Length > 0)
                        sb.Length--;
